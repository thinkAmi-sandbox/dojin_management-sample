...
describe('Event Exhibits Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await drizzleService.db.insert(schema.exhibits).values({
      ...
      circleId: circle.id,
      status: 'applied',
      applicationNotes: '申込備考',
    })
  })

  describe('GET /events/:eventId/exhibits', () => {
    it('イベント別出展一覧が表示される', async () => {
      ...
      expect(response.text).toContain('テストサークル')
      expect(response.text).toContain('申込中')
    })
  })

  describe('GET /events/:eventId/exhibits/new', () => {
    it('イベントへの新規出展申込画面が表示される', async () => {
      const eventsData = await drizzleService.db.select().from(schema.events)
      const eventId = eventsData[0].id

      const response = await request(app.getHttpServer())
        .get(`/events/${eventId}/exhibits/new`)
        .expect(200)

      expect(response.text).toContain('技術書典17への出展申込')
      expect(response.text).toContain('申込サークル')
      expect(response.text).toContain('申込備考')
    })
  })

  describe('POST /events/:eventId/exhibits', () => {
    it('イベントへの新規出展申込が作成される', async () => {
      const eventsData = await drizzleService.db.select().from(schema.events)
