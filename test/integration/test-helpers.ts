 ...
 * 全てのテーブルのデータを削除する
 */
export async function cleanupDatabase(db: NodePgDatabase<typeof schema>): Promise<void> {
  // 全テーブルのデータを削除（強制実行）
 ...
 */
export async function initializeTestDatabase(db: NodePgDatabase<typeof schema>): Promise<void> {
  // 強制的にすべてのデータを削除
  await db.delete(schema.books).execute()
